...
namespace WebSite.EndPoint.Areas.Admin.Controllers
{
    ...
    public class CategoryController : Controller
    {
        ...
        [HttpPost]
        public Task<IActionResult> Save(int id, string name, string img)
        {
            if (id > 0)
                return Task.FromResult<IActionResult>(RedirectToAction(nameof(Edit), new { id, name, img }));
            else
                return Task.FromResult<IActionResult>(RedirectToAction(nameof(Create), new { name, img }));
        }
    }
}